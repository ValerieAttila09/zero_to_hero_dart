...
class User {
  ...
  factory User.fromMySQL(ResultRow row) {
    return User(
      id: row['id'] as String,
      nama: row['nama'] as String,
      umur: row['umur'] as int,
...
class UserRepository {
  ...
  Future<bool> addUser(User user) async {
    ...
    try {
      await _connection.query(
        'INSERT INTO users (id, nama, umur, email) VALUES (?, ?, ?, ?)',
  ...
  Future<bool> updateUser(User user) async {
    ...
    try {
      final result = await _connection.query(
        'UPDATE users SET nama = ?, umur = ?, email = ? WHERE id = ?',
        [user.nama, user.umur, user.email, user.id],
      );
      return result.affectedRows! > 0;
    } catch (e) {
  ...
  Future<User?> findUserById(String id) async {
    ...
    try {
      final results = await _connection.query(
        'SELECT id, nama, umur, email FROM users WHERE id = ?',
        [id],
      );
      ...
      return User.fromMySQL(results.first);
    } catch (e) {
